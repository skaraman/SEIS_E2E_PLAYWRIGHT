...
export const generateAttendanceRegister = async (page: Page) => {
	...
	await clickElement(page, page.getByRole('link', { name: 'Last Name' }));
	await clickElement(page, page.getByRole('option', { name: 'First Name' }));
	await clickElement(page, page.getByRole('button', { name: 'Print' }));
	await clickElement(page, page.getByLabel('Automatically Number Pages, ex: Page 1 of 14'));
	await clickElement(page, page.getByLabel(/Remove\s+"Page\s+___\s+of\s+___"\s+Text/));
