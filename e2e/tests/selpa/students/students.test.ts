...
import { verifyIfTitleIsCorrect } from '../../../helpers/verify'
import { clickElement } from '../../../helpers/actions'
import { locators } from '../../../components/navigation-bar/students-menu-drop-down'
import { addStudentPage, exitedStudentsPage, futureIepFormsPage, studentDemographicsPage, studentIepsPage } from '../../../components/students/index'
import { addNewStudent, requestTransfer } from '../../../components/students/add-student.page'
...
import { generateEsignaturePreMeeting, validateEsignaturePage, generateEsignatureCompleted } from '../../../components/students/future-iep-forms.page'
...
test.describe('SELPA > Students Load Tests', () => {
  ...
  test('Add Students Request Transfer @HD-Test', async ({ page }) => {
    ...
    await clickElement(page, page.getByRole('button', { name: 'Edit Student' }))
    await clickElement(page, locators.STUDENTS, 0, 'text')
    await clickElement(page, locators.ADD_STUDENT)
  ...
  test('Quick Links Verify @HD-Test', async ({ page }) => {
    ...
    await clickElement(page, page.getByRole('link', { name: 'Demographics' }))
    await waitForPageReady(page)
  ...
  test.skip('Generate E-Signature Pre-meeting @HD-Test', async ({ page, request }, configs) => {
    await clickElement(page, studentsMenuDropDown.locators.STUDENTS, 0, 'text')
    await clickElement(page, studentsMenuDropDown.locators.STUDENT_IEPS, 0, 'text')
    await page.waitForSelector(studentIepsPage.locators.TABLE)
    await selectEligibility(page)
  ...
  test.skip('Generate E-Signature Complete @HD-Test', async ({ page, request }, configs) => {
    await clickElement(page, studentsMenuDropDown.locators.STUDENTS, 0, 'text')
    await clickElement(page, studentsMenuDropDown.locators.STUDENT_IEPS, 0, 'text')
    await page.waitForSelector(studentIepsPage.locators.TABLE)
